...
public class Native {
    ...
    public static native byte[] execute(long vm, StateContext context, int revision, byte[] msg, byte[] code);
    ...
    public static int account_exists(StateContext context, byte[] address) {
        return context.accountExists(Address.warp(address)) ? 1 : 0;
    }

    public static void get_storage(byte[] result, StateContext context, byte[] address, byte[] key) {
    ...
    public static void set_storage(StateContext context, byte[] address, byte[] key, byte[] value) {
        context.setStorage(Address.warp(address), DataWord.wrap(key), DataWord.wrap(value));
    }

    public static void get_balance(byte[] result, StateContext context, byte[] address) {
    ...
    public static int get_code_size(StateContext context, byte[] address) {
        byte[] code = context.getCode(Address.warp(address));

        return (code == null) ? 0 : code.length;
    }

    public static int copy_code(StateContext context, byte[] address, int offset, byte[] buffer) {
        byte[] code = context.getCode(Address.warp(address));
    ...
    public static void selfdestruct(StateContext context, byte[] address, byte[] beneficiary) {
        context.selfdestruct(Address.warp(address), Address.warp(beneficiary));
    }

    public static void emit_log(StateContext context, byte[] address, byte[] data, byte[] topics) {
        ...
        context.emitLog(Address.warp(address), arr, data);
    }

    public static byte[] call(StateContext context, byte[] msg) {
